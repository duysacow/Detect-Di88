...
class ROIBox(QFrame):
    def __init__(self, parent, name, rect, color="#00FF00"):
        ...
        self.label.setStyleSheet(f"color: {color}; font-weight: bold; font-size: 10px; background: rgba(0,0,0,100);")
        ...
        self.resize_handle.setStyleSheet(f"color: {self.color}; font-size: 14px; font-weight: bold;")
    ...
    def update_handle_pos(self):
        if hasattr(self, 'resize_handle'):
            self.resize_handle.setGeometry(self.width()-15, self.height()-15, 15, 15)
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.MouseButton.LeftButton:
            ...
            pos = event.pos()
            if pos.x() > self.width() - self.resize_margin and pos.y() > self.height() - self.resize_margin:
                self.resizing = True
            else:
                self.dragging = True
...
class ToolOverlayROI(QWidget):
    ...
    def init_ui(self):
        ...
        self.panel.setStyleSheet("background-color: #222; border: 2px solid #555; border-radius: 10px;")
        
        p_layout = QHBoxLayout(self.panel)
        
        lbl_msg = QLabel(f"DI88 OVERLAY ROI TOOL ({self.width()}x{self.height()})\nDrag boxes, Resize at bottom-right corner", self.panel)
        ...
        btn_close.setStyleSheet("background-color: #AA0000; color: white; font-weight: bold; border-radius: 5px;")
...
'''
            ...
            self.panel.findChildren(QLabel)[0].setText(f"SAVED [{res_key}] SUCCESSFULLY!\nRestart Macro to apply.")
                
        except Exception as e:
            import traceback
            traceback.print_exc()
            print(f"[ERROR] Save failed: {e}")
            self.panel.findChildren(QLabel)[0].setText(f"SAVE FAILED!\nError: {str(e)[:25]}...")
