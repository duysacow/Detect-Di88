from PyQt6.QtWidgets import (QApplication, QMainWindow, QWidget, QLabel, QVBoxLayout, QHBoxLayout, 
                             QPushButton, QFrame, QGridLayout, QGroupBox, QComboBox, 
...
class MacroWindow(QMainWindow):
    ...
    def set_capture_mode_ui(self, mode):
        ...
        # 1. Update Properties
        self.btn_mode_dxcam.setProperty("active", "true" if mode == "DXCAM" else "false")
    ...
    def load_crosshair_settings(self):
        ...
        try:
            ...
            if idx == -1: idx = 0 # Default fallback
    ...
    def reset_to_defaults(self):
        ...
        try:
            ...
            if hasattr(self, 'combo_color') and self.combo_color:
                ...
                color_idx = {'Red': 0, 'Green': 1, 'Blue': 2, 'Cyan': 3, 'Yellow': 4, 'Magenta': 5, 'White': 6}
            ...
            pass
            QMessageBox.information(self, 'Xong!', 'Reset thành công! Tất cả đã về mặc định.')
        except Exception as e:
    ...
    def update_macro_style(self, is_on):
        ...
        if is_on:
            ...
            self.btn_macro.setStyleSheet(f"QPushButton {{ color: #00FFFF; background: #0d1f1f; border: 1px solid #006666; {base} }}")
        else:
            self.btn_macro.setText("MACRO : OFF")
            self.btn_macro.setStyleSheet(f"QPushButton {{ color: #ff4444; background: #1a1010; border: 1px solid #441111; {base} }}")
    ...
    def update_ui_state(self, data):
        ...
        self.panel_g1.setStyleSheet(f"QFrame#P1 {{ background-color: #262626; border-radius: 8px; {static_border} }}")
        self.panel_g2.setStyleSheet(f"QFrame#P2 {{ background-color: #262626; border-radius: 8px; {static_border} }}")

        def item_style(lbl, val):
            if val == "NONE":
                lbl.setStyleSheet("background-color: #2e2e2e; color: #666; border-radius: 4px; padding: 2px; font-size: 11px;")
            else:
                lbl.setStyleSheet("background-color: #2e2e2e; color: #FFD700; border-radius: 4px; padding: 2px; font-size: 11px; font-weight: bold;")
