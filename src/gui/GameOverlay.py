...
class GameOverlay(QWidget):
    def __init__(self, parent=None):
        ...
        self.lbl.setStyleSheet("color: #00FF00; font-weight: 900; font-family: 'Segoe UI'; font-size: 10px; border: none; background: transparent;")
    ...
    def _do_flash(self):
        ...
        self.lbl.setStyleSheet(f"color: {color}; font-weight: 900; font-family: 'Segoe UI'; font-size: 10px; border: none; background: transparent;")
        # VIỀN GIỮ NGUYÊN (Theo ý sếp)
        self.frame.setStyleSheet("QFrame { background-color: rgba(10, 10, 10, 180); border: 1px solid #444444; border-radius: 6px; }")
        self.color_idx += 1

    def _do_detect_flash(self):
        """Hàm xử lý chớp cam khi AI đang ACTIVE"""
        if self.flash_timer.isActive(): return # Đang bắn thì không nháy cam đè lên
    ...
    def update_status(self, gun_name, scope, stance, grip="NONE", muzzle="NONE", is_paused=False, is_firing=False, ai_status="HIBERNATE"):
        ...
        if is_paused:
            ...
            sc_val = "X1"
            if "2" in scope_raw: sc_val = "X2"
            elif "3" in scope_raw: sc_val = "X3"
            elif "4" in scope_raw: sc_val = "X4"
            elif "6" in scope_raw: sc_val = "X6"
            elif "8" in scope_raw: sc_val = "X8"
            
            if "kh" in scope_raw: sc_val = f"KH {sc_val}"

            vn_stance = stance
            if "STAND" in stance.upper(): vn_stance = "ĐỨNG"
            elif "CROUCH" in stance.upper(): vn_stance = "NGỒI"
            elif "PRONE" in stance.upper(): vn_stance = "NẰM"

            parts = [str(gun_name).upper(), sc_val]
            if str(grip).upper() != "NONE": parts.append("TAY")
            if str(muzzle).upper() != "NONE": parts.append("NÒNG")
