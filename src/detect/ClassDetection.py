...
class DetectionEngine:
    ...
    def detect_accessory(self, roi, threshold=0.8):
        ...
        name = res.upper()
        if "GIAMGIAT" in name: return "GiamGiat"
        if "ANTIALUA" in name: return "AnTiaLua"
        if "GIAMRUNG" in name: return "GiamRung"
        if "ATLSMG" in name: return "ATLsmg"
        if "GGIATSMG" in name: return "GGiatSMG"
        if "GTHANHSMG" in name: return "GThanhSMG"
        return res
    ...
    def detect_grip(self, roi, threshold=0.8):
        ...
        name = res.upper()
        if "TCDUNG" in name: return "tcDung"
        if "TCHONG" in name: return "tcHong"
        if "TCLASER" in name: return "tcLaser"
        if "TCNAMCHAT" in name: return "tcNamChat"
        if "TCNGHIENG" in name: return "tcNghieng"
        if "TCNHE" in name: return "tcNhe"
        return res
    ...
    def detect_scope(self, roi, threshold=0.8):
        ...
        if "SCOPEKH" in name: return "ScopeKH"
    ...
    def detect_stance(self, roi, threshold=0.6):
        ...
        if res == "NONE": return "Stand"
        
        name = res.upper()
        if "STANDING" in name: return "Stand"
        if "CROUCHING" in name: return "Crouch"
