...
class ScreenCapture:
    ...
    def grab_regional_image(self):
        ...
        if self.capture_mode == "DXCAM" and self.camera:
            l, t, w, h = self.bbox["left"], self.bbox["top"], self.bbox["width"], self.bbox["height"]
            region = (l, t, l + w, t + h)
    ...
    def get_roi_from_image(self, regional_img, roi_name):
        ...
        if not roi or regional_img is None: return None
    ...
    def debug_rois(self):
        ...
        if not hasattr(self, 'bbox'): self.calculate_bounding_box()
