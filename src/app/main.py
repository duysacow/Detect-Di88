...
def _optimize_cpu_and_priority():
    ...
    try:
        ...
        if count > 1:
            ...
            print(f" > [SYSTEM] CPU Isolated: Pinned to Core {count - 1} (Avoid FPS Drops)")
...
if __name__ == "__main__":
    ...
    class InputBridge(QObject):
        def __init__(self, window, backend):
            ...
            self._ads_reset_timer = None  # Timer tự reset is_ads sau khi thả LMB trong TOGGLE
            self.reload_config()
        ...
        def handle_input_action(self, action):
            ...
            if action == "SLOT_1": self.backend.set_slot(1); self.window.update_macro_style(True)
            elif action == "SLOT_2": self.backend.set_slot(2); self.window.update_macro_style(True)
            
            if not Utils.is_game_active(): return
            if action == "MACRO_PAUSE": self.backend.set_paused(True); self.window.update_macro_style(False)
        ...
        def handle_raw_key(self, key, pressed):
            if key == self.guitoggle_key and pressed:
                ...
                return
            if pressed and not Utils.is_game_active(): return
            elif key == "f2" and pressed: self.backend.reload_config()
        ...
        def handle_mouse_click(self, btn, pressed):
            ...
            if btn_name == "right":
                ...
                pubg_ads = getattr(self.backend.pubg_config, 'ads_mode', None)
                ads_mode = pubg_ads.upper() if pubg_ads else self.settings.get("ads_mode", "HOLD").upper()
                if ads_mode == "HOLD": self.is_ads = pressed
            ...
            if btn_name == "left" and pressed:
                ...
                if not Utils.is_game_active() or cursor_visible: return
                ...
                if data.get("paused", False): return
