...
class PubgConfig:
    ...
    def parse_config(self):
        ...
        try:
            ...
            try:
                with open(
                    self.config_path, "r", encoding="utf-8", errors="ignore"
                ) as f:
                    content = f.read()
            ...
            v_matches = list(
                re.finditer(
                    r"MouseVerticalSensitivityMultiplierAdjusted=([\d\.]+)", content
                )
            )
            if v_matches:
            ...
            mouse_matches = list(
                re.finditer(r"\(Mouse, \(Array=\((.*?)\)\)\)", content)
            )
            new_sens = {}
            true_sens_map = {}
            ...
            if mouse_matches:
                ...
                for match in re.finditer(
                    r'SensitiveName="([^"]+)",Sensitivity=([\d\.]+),LastConvertedSensitivity=([\d\.]+)',
                    mouse_block,
                ):
                    name = match.group(1)
            ...
            per_scope_matches = re.findall(
                r"bIsUsingPerScopeMouseSensitivity=(True|False)", content
            )
            self.per_scope_enabled = False
